...
function main()
    df = collect_results(datadir("PulseGhzHeatmaps"))

    mkpath(plotsdir("PulseGhzHeatmaps")) # Where we will store plots
    ...
    for df_nqubits in @groupby(df, :Nqubits, :Npoints, :i1, :i2, :optimizer)
        ...
        dict_inf = @dict(Nqubits, i1, i2, Npoints, controlFuncType, controlPermType, J, T, cost="infidelity")
        ...
        save(plotsdir("PulseGhzHeatmaps", savename(dict_inf, "png")), fig_inf)

        if !(optimizer == "none") # Only make W1 plot if W1 distances were computed
            data_W1 = df_nqubits[1,:data_W1]
            dict_W1 = @dict(Nqubits, i1, i2, Npoints, controlFuncType, controlPermType, J, T, cost="W1")
            ...
            save(plotsdir("PulseGhzHeatmaps", savename(dict_W1, "png")), fig_W1)
        end
    end
end
